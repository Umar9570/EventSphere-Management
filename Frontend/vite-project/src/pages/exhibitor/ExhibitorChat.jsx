...
const ExhibitorChat = () => {
    ...
    useEffect(() => {
        ...
        s.on("receiveMessage", (msg) => {
            ...
            setChatList((prevList) => {
                ...
                return sortChats(updated); //  move chat to top instantly
            });
            ...
            try {
                ...
                const chats = await Promise.all(
                    participants.map(async (participant) => {
                        ...
                        const expoQuery =
                            participant.role === "organizer" ? "" : `&expo=${expoId}`;

                        const lastMsgRes = await api.get(
                            `/messages/conversation?user1=${user.id}&user2=${participant._id}${expoQuery}`
                        );
                        ...
                        const orgName = participant.orgName || "";


                        return { user: participant, lastMessage: lastMsg, lastMessageObj: lastMsgObj, badge: unreadCount, orgName };
                    })
                );

                // Add timestamp from last message so we can sort
                const chatsWithTimes = chats.map((chat, index) => {
                    return {
                        ...chat,
                        lastMessageTime: chat.lastMessageObj?.createdAt || 0
                    };
                });

                // Sort by most recent activity (WhatsApp style)
                const sortedChats = chatsWithTimes.sort((a, b) => {
    ...
    const selectChat = async (chat) => {
        ...
        const expoQuery =
            chat.user.role === "organizer" ? "" : `&expo=${expoId}`;

        try {
            const { data } = await api.get(
                `/messages/conversation?user1=${user.id}&user2=${chat.user._id}${expoQuery}`
            );
            ...
            // Clear badge for the selected chat and keep sorting intact
            setChatList((prev) => {
                const updated = prev.map((c) =>
                    c.user && c.user._id === chat.user._id
                        ? { ...c, badge: 0 }
                        : c
                );
    ...
    const sendMessage = async () => {
        ...
        try {
            ...
            if (data.status) {
                ...
                // Update chat list: update lastMessage + lastMessageObj and re-sort
                setChatList((prev) => {
                    const updated = prev.map((c) =>
                        c.user && c.user._id === selectedChat.user._id
                            ? {
                                ...c,
                                lastMessage: sentMessage.content,
                                lastMessageObj: sentMessage,
                            }
                            : c
                    );
    ...
    const markAsDelivered = async (messageIds, senderId) => {
        ...
        try {
            await api.put("/messages/delivered", {
                messageIds,
                userId: user.id,
            });

            if (socketRef.current) {
                socketRef.current.emit("markDelivered", {
                    messageIds,
                    userId: user.id,
                    senderId,
                });
                                    ...
                                    <Col md={6} lg={5} xl={4} className="mb-4 mb-md-0">
                                        <div className="p-3">
                                            <InputGroup className="rounded mb-3">
                                                {/* 🔍 SEARCH INPUT (UPDATED ONLY THIS) */}
                                                <Form.Control
                                            ...
                                            >
                                                <ul className="list-unstyled mb-0">
                                                    ...
                                                    {filteredChatList.map((chat, idx) => (
                                                        <li
                                                            className={`p-2 border-bottom chat-list ${selectedChat &&
                                                            ...
                                                            <div className="d-flex justify-content-between text-dark">
                                                                <div className="d-flex flex-row">
                                                                    ...
                                                                    <div className="pt-1">
                                                                        ...
                                                                        <p className="small text-muted mt-0 pt-0">
                                                                            {chat.lastMessage}
                                                                        </p>
                                                                    </div>
                                                                </div>
                                        ...
                                        >
                                            {!selectedChat ? (
                                                ...
                                                messages.map((msg, idx) => (
                                                    <div
                                                        key={msg._id || idx}
                                                        className={`d-flex flex-row justify-content-${msg.sender._id === user.id
                                                            ? "end"
                                                            : "start"
                                                            } mb-3`}
                                                    >
                                                        ...
                                                        <div>
                                                            ...
                                                            <p
                                                                className={`small mb-3 rounded-3 text-muted ${msg.sender._id === user.id
                                                                    ? "text-end"
                                                                    : ""
                                                                    }`}
                                                            >
